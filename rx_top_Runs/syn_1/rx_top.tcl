...
set rc [catch {
  ...
  elaborate -top {rx_top}
  export_db {rx_top_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../rx_top.adc
  optimize_rtl
  report_area -file rx_top_rtl.area
  export_db {rx_top_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea rx_top_gate.area
  legalize_phy_inst
  export_db {rx_top_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
