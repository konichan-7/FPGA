...
set rc [catch {
  open_project {rx_top.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/rx_top_gate.db}
  config_chipwatcher "../../tx.cwc" -dir ""
  compile_watcher
  place
  export_db {rx_top_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file rx_top_phy.area
  export_db {rx_top_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid rx_top_inst.bid
  bitgen -bit "rx_top.bit" -unused_io_status pulldown
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
